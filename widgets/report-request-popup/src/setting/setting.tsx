...
import '../runtime/css/style.css' // custom css styling
...
const Setting = (props: AllWidgetSettingProps<IMConfig>) => {
  ...
  // The user needs to configure the map widget that this custom widget will interact with
    ...
    const [enableGeoJsonRadioBtn, setEnableGeoJsonRadioBtn] = useState(props.config?.enableRadioBtns || false);

    // custom setting to enable radio buttons with geoJson override option. this setting should only be checked for the internal Science division web app
    const [isInternalApp, setIsInternalAppn] = useState(props.config?.internalApp || false);

    // useEffect(() => {
    //   // Update states if config changes from previous app owner session
    //   if (props.config.appName !== inputValue) {
    //     setInputValue(props.config?.appName || '');
    //   }
    //   if (props.config.geoJsonOverride !== enableGeoJsonRadioBtn) {
    //     setEnableGeoJsonRadioBtn(props.config?.geoJsonOverride || false);
    //   }
    // }, [props.config.appName, props.config.geoJsonOverride]);
    ...
    const handleRadioChange = (event: React.ChangeEvent<HTMLInputElement>, checked: boolean) => {
      setEnableGeoJsonRadioBtn(checked);
      props.onSettingChange({
        id: props.id,
        config: props.config.set('enableRadioBtns', checked),
      });
    };
      ...
      <SettingSection
        ...
        <SettingRow>
          ...
          <Switch
            ...
            checked={enableGeoJsonRadioBtn}
            onChange={handleRadioChange}
            title='Show radio buttons'
          />
        </SettingRow>
        <SettingRow>
          <span className='popup-report-request-padding-right'>
